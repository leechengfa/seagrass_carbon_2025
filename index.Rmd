...
.xlab <- expression('Leaf biomass (g/m^2)')
.ylab <- expression('Root biomass (g/m^2)')

leaf_root_slope <- 0.63
leaf_root_intercept <- 8.5
...
csv.biomass.AV$predicted_BGB <- 0.89*csv.biomass.AV$seagrassA+2.5
...
csv.biomass.AV$sectoring <- paste0(
  case_when(
    csv.biomass.AV$seagrassA < 50 ~ 'a',
    csv.biomass.AV$seagrassA < 100 ~ 'b',
    csv.biomass.AV$seagrassA < 150 ~ 'c',
    csv.biomass.AV$seagrassA < 200 ~ 'd',
    csv.biomass.AV$seagrassA < 250 ~ 'e',
    csv.biomass.AV$seagrassA >= 300 ~ 'f'
  ),
  case_when(
    csv.biomass.AV$seagrassA < 50 ~ 'a',
    csv.biomass.AV$seagrassB < 100 ~ 'b',
    csv.biomass.AV$seagrassB < 150 ~ 'c',
    csv.biomass.AV$seagrassB < 200 ~ 'd',
    csv.biomass.AV$seagrassB < 250 ~ 'e',
    csv.biomass.AV$seagrassB >= 300 ~ 'f'
  )
)
...
plt.score.selected <- ggplot(csv.biomass.AV %>% filter(name %in% selected | !is.na(csv.biomass.AV$`spectral camera`)), aes(x = seagrassA, y = seagrassB)) +
  geom_rect(aes(xmin = 0, xmax = recommended_minimum, ymin = 0, ymax = recommended_minimum), alpha = 0.5, fill = 'lightblue') +
  geom_smooth(method = 'lm') +
...
csv.biomass.BB$sectoring <- paste0(
  case_when(
    csv.biomass.BB$A < 50 ~ 'a',
    csv.biomass.BB$A > 50 & csv.biomass.BB$A < 100 ~ 'b',
    csv.biomass.BB$A > 100 & csv.biomass.BB$A < 150 ~ 'c',
    csv.biomass.BB$A > 150 & csv.biomass.BB$A < 200 ~ 'd',
    csv.biomass.BB$A > 200 ~ 'e'
  ),
  case_when(
    csv.biomass.BB$B < 50 ~ 'a',
    csv.biomass.BB$B > 50 & csv.biomass.BB$B < 100 ~ 'b',
    csv.biomass.BB$B > 100 & csv.biomass.BB$B < 150 ~ 'c',
    csv.biomass.BB$B > 150 & csv.biomass.BB$B < 200 ~ 'd',
    csv.biomass.BB$B > 200 ~ 'e'
  )
)
...
## checking if the linear relationship between AGB and BGB is substantially changed after the data thinning
## representation is similar
plt.score.selected <- ggplot(csv.biomass.BB %>% filter(Station %in% selected | spectra == TRUE), aes(x = A, y = B)) +
  geom_rect(aes(xmin = 0, xmax = recommended_minimum, ymin = 0, ymax = recommended_minimum), alpha = 0.5, fill = 'lightblue') +
  geom_smooth(method = 'lm') +
