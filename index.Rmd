...
csv.carbon <- rbind(csv.carbon.AV, csv.carbon.CB) 
  ...
  csv.carbon$Group <- factor(csv.carbon$Group, levels = c('Natural', 'Rewilded', 'spc_0', 'spc_50', 'spc_100'), labels = c('Natural\n(Aveiro)', 'Rewilded\n(Aveiro)', '0% seagrass\ncover\n(Cadiz)', '50% seagrass\ncover\n(Cadiz)', '100% seagrass\ncover\n(Cadiz)'))

## to work on the dry weight correction: does not affect content values; affects stock values  
  
csv.carbon <- csv.carbon %>% 
  filter(!is.na(DryMassTotal)) %>%
  mutate(# sample dry weight correction (simple version; not separated by site)
         DryMassIC = DryMassIC*0.9,
         # carbon content
         TC_gG = MeasuredTC/DryMassTC, TIC_gG = MeasuredIC/DryMassIC, TOC_gG = TC_gG - TIC_gG, 
...
ggplot(csv.carbon.ratio, aes(x = Group, y = AgbBgbRatio)) +
  geom_boxplot() +
...
```{r DWAgbBgbRatioScatterplot}
#| echo: false
#| error: false
#| message: false
#| warning: false
#| fig-align: center
#| fig-cap: Scatterplot showing the seagrass aboveground and belowground dry weight (g) of the cores across different conditions against their corresponding AGB/BGB ratio.

plt.DW_AgbBgbRatio <- ggplot(csv.carbon, aes(x = DryMassTotalPerM2, y = AgbBgbRatio)) +
  ...
  xlab(.dwPerArea) + ylab(.AgbBgbRatio) +
  facet_grid(. ~ PartType)

girafe(ggobj = plt.DW_AgbBgbRatio,
         options = list(
...
#| warning: false
#| fig-align: center
#| fig-cap: Scatterplot showing the seagrass aboveground and belowground organic carbon stock (C g per unit area) of the cores across different conditions.
