...
csv.biomass.CB <- read_xlsx('data/biomass/Cadiz_Biomass_C_cores.xlsx') %>%
  ...
  mutate(type = case_when(# High cover
         ...
         name = paste(type, Sample, sep = '_')
         )

csv.biomass.CB$Leaves <- csv.biomass.CB$Leaves * 1000  # conversion to mg
csv.biomass.CB$Roots <- csv.biomass.CB$Roots * 1000  # conversion to mg
...
table.amtSmallFieldSamples.CB <- csv.biomass.CB %>% 
  filter(Leaves < 370 | Roots < 370) %>%
  arrange(name) %>%
  select(name, Leaves, Roots)

knitr::kable(table.amtSmallFieldSamples.CB %>% rename(c(Station = name, "Collected leaf biomass (mg)" = Leaves, "Collected root biomass (mg)" = Roots)), 
             format = "pipe",
             align = 'c',
...
#| message: false
#| warning: false
#| fig.cap: Data exploration for the Cadiz Bay samples. The light blue rectangle denotes that the station has insufficient biomass for the carbon measurement, based on the requirements outlined in section \@ref(toc-measurement). All plots establishes the general relationship between leaf and root biomass for (A) all stations, (B) bare and intermediate cover stations only, and (C) high cover stations only. The black dotted line represents the overall linear relationship between leaf and root biomass across all three locations.
...
plt.leaf_root <- ggplot(csv.biomass.CB, aes(x = Roots, y = Leaves)) +
...
plt.leaf_root_non_high <- ggplot(csv.biomass.CB %>% filter(type != "High"), aes(x = Roots, y = Leaves)) +
  geom_rect(aes(xmin = 0, xmax = 370, ymin = 0, ymax = 370), alpha = 0.5, fill = 'lightblue') +
  geom_smooth(method = 'lm') + 
  geom_abline(intercept = 13, slope = 0.9, linetype = 'dashed') + ## based on combined results
...
plt.leaf_root_high <- ggplot(csv.biomass.CB %>% filter(type == "High"), aes(x = Roots, y = Leaves)) +
  geom_rect(aes(xmin = 0, xmax = 370, ymin = 0, ymax = 370), alpha = 0.5, fill = 'lightblue') +
  geom_smooth(method = 'lm') + 
  geom_abline(intercept = 13, slope = 0.9, linetype = 'dashed') + ## based on combined results
  geom_point_interactive(aes(tooltip = paste0("Station: ", name), data_id = name, colour = type), show.legend = FALSE) +
...
girafe(ggobj = plt.leaf_root + plt.leaf_root_non_high + plt.leaf_root_high + plot_annotation(tag_levels = 'A') + plot_layout(guides = 'collect', axis_titles = 'collect'),
         options = list(
           opts_hover(css = "stroke-width:2.5px"),
           opts_hover_inv(css = "opacity:0.25"),
           opts_tooltip(opacity = 0.95, css = "padding:6px; font-size:12px")
         ),
         height_svg = 7, width_svg = 12
  )
```
...
Of the stations with hyperspectral measurements, `r csv.biomass.AV %>% filter(seagrassA < 370 | seagrassB < 370) %>% select(spectra) %>% sum()` have too little samples for the carbon measurement (more in section \@ref(toc-measurement)). They are:
...
table.amtSmallFieldSamples.AV <- csv.biomass.AV %>% 
  filter(seagrassA < 370 | seagrassB < 370) %>%
  filter(spectra == TRUE) %>%
  arrange(name) %>%
  select(name, seagrassA, seagrassB)

knitr::kable(table.amtSmallFieldSamples.AV %>% rename(c(Station = name, "Collected leaf biomass (mg)" = seagrassA, "Collected root biomass (mg)" = seagrassB)), 
             format = "pipe",
             align = 'c',
...
Table \@ref(tab:amountSmallFieldSamplesAV) shows that `r nrow(filter(table.amtSmallFieldSamples.AV, seagrassA + seagrassB == 0))` stations have no samples at all and `r nrow(filter(table.amtSmallFieldSamples.AV, seagrassA + seagrassB != 0 & seagrassA == 0 | seagrassA + seagrassB != 0 & seagrassB == 0))` station have no leaf or root samples for sampling. Based on the stated criteria, these stations were removed for the list of viable stations for carbon sampling. For the remaining `r nrow(table.amtSmallFieldSamples.AV) - nrow(filter(table.amtSmallFieldSamples.AV, seagrassA + seagrassB == 0)) - nrow(filter(table.amtSmallFieldSamples.AV, seagrassA + seagrassB != 0 & seagrassA == 0 | seagrassA + seagrassB != 0 & seagrassB == 0))` stations, NR 43 could still be sampled as usual, with its root biomass sampling having close to the recommended 300 mg for inorganic carbon content sampling. Meanwhile, RG 22 and RG 46 are both in the rewilded site and have very similar spectra. As such, they could be pooled together as one for the carbon sampling. 
...
#| message: false
#| warning: false
#| fig.cap: Data filtering for the Ria de Aveiro samples. The light blue rectangle denotes that the station has insufficient biomass for the carbon measurement, based on the requirements outlined in section \@ref(toc-measurement). (A) establishes the general relationship between leaf and root biomass. (B) scores the non-zero stations based on the established criteria. (C) demonstrates that the filtered data's relationship between leaf and root biomass is somewhat preserved with its similar slope factor to A (represented by the blue dotted line). The black dotted line represents the overall linear relationship between leaf and root biomass across all three locations.
...
csv.biomass.AV$sectoring <- paste0(
  case_when(
    csv.biomass.AV$seagrassA < 500 ~ 'a',
    csv.biomass.AV$seagrassA < 1000 ~ 'b',
    csv.biomass.AV$seagrassA < 2000 ~ 'c',
    csv.biomass.AV$seagrassA < 3000 ~ 'd',
    csv.biomass.AV$seagrassA < 4000 ~ 'e',
    csv.biomass.AV$seagrassA >= 4000 ~ 'f'
  ),
  case_when(
    csv.biomass.AV$seagrassA < 500 ~ 'a',
    csv.biomass.AV$seagrassB < 1000 ~ 'b',
    csv.biomass.AV$seagrassB < 2000 ~ 'c',
    csv.biomass.AV$seagrassB < 3000 ~ 'd',
    csv.biomass.AV$seagrassB < 4000 ~ 'e',
    csv.biomass.AV$seagrassB >= 4000 ~ 'f'
  )
)
...
Of the selected, RG 17 and RG 24 have only 250 mg and 338 mg of root samples, respectively. As with NR 43, RG 24 has almost enough root sample and can be performed as it is. Meanwhile, RG 17 will be pooled with its neighbour RG 9 to attain the recommended sample weight.
...
Of the stations with hyperspectral measurements, `r csv.biomass.BB %>% filter(A < 370 | B < 370) %>% select(spectra) %>% sum()` stations have too little samples for the carbon measurement (more in section \@ref(toc-measurement)). They are:
...
knitr::kable(table.amtSmallFieldSamples.BB %>% rename(c("Collected leaf biomass (mg)" = A, "Collected root biomass (mg)" = B)), 
             format = "pipe",
             align = 'c',
...
Normally, this would means that `r 30 - sum(csv.biomass.BB$spectra)` samples need to be selected from the remaining `r sum(table.amtFieldBB['Actual no. of field stations']) - sum(csv.biomass.BB$spectra) - nrow(filter(csv.biomass.BB, A + B == 0)) - nrow(filter(csv.biomass.BB, is.na(B)))` samples which have neither hyperspectral measurements nor have no seagrass biomass. However, table \@ref(tab:amountSmallFieldSamplesBB) shows that `r csv.biomass.BB %>% filter(spectra == TRUE) %>% filter(A < 370 | B < 370) %>% nrow()` samples have less than 370 mg for either the leaf or the root biomass or both. Of these samples, only H_17 could be measured as usual, since its weight is close to the recommended 370 mg. Arguably, the same risk could be taken for H_23. Although H_21, H_22, and H_23 are neighbouring points, they have very different spectra.
...
#| message: false
#| warning: false
#| fig.cap: Data filtering for the Bourgneuf Bay samples. The light blue rectangle denotes that the station has insufficient biomass for the carbon measurement, based on the requirements outlined in section \@ref(toc-measurement). (A) establishes the general relationship between leaf and root biomass. (B) scores the non-zero stations based on the established criteria. (C) shows the change in the relationship between leaf and root biomass between the original data (represented by the blue dotted line) and the filtered data. The black dotted line represents the overall linear relationship between leaf and root biomass across all three locations.
